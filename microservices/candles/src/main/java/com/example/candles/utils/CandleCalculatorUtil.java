...
public class CandleCalculatorUtil {
     ...
     */
    public static int calculateCandlesCount(String timeframe, String period) {
        log.debug("🧮 РАСЧЕТ СВЕЧЕЙ: timeframe={}, period={}", timeframe, period);
        
        try {
            int periodDays = parsePeriodToDays(period);
            int candlesCount = calculateCandlesByTimeframe(timeframe, periodDays);
            
            log.info("✅ РЕЗУЛЬТАТ РАСЧЕТА: {} свечей для периода '{}' ({} дней) с таймфреймом {}", 
                    candlesCount, period, periodDays, timeframe);
            return candlesCount;
            
        } catch (Exception e) {
            log.error("❌ ОШИБКА РАСЧЕТА: Ошибка при расчете свечей для timeframe={}, period={}: {}", 
