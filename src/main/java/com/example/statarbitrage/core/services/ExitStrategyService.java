...
public class ExitStrategyService {
    ...
    public String getExitReason(PairData pairData, Settings settings) {
        double profit = pairData.getProfitPercentChanges().doubleValue(); //todo здесь берем из changes-ов, а надо заменить на calcProfit
        ...
        if (settings.isUseExitZMin() && zScoreCurrent <= settings.getExitZMin()) {
            log.info("ExitStrategyService: Определили выход по zMin: zMin = {}", zScoreCurrent);
        ...
        if (settings.isUseExitTimeHours() && entryTimeMillis > 0) {
            ...
            if (holdingHours >= settings.getExitTimeHours()) {
                log.info("ExitStrategyService: Определили выход по времени: ожидали {} часов", holdingHours);
