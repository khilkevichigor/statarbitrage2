...
/**
 * Страница закрытых пар
 */
@Slf4j
@PageTitle("Закрытые пары")
@Route(value = "closed-pairs", layout = MainLayout.class)
public class ClosedPairsView extends VerticalLayout implements UIUpdateable {

    private final TradingPairsComponent tradingPairsComponent;
    private final UIUpdateService uiUpdateService;

    public ClosedPairsView(TradingPairsComponent tradingPairsComponent,
    ...
    private void initializeLayout() {
        add(new H2("Закрытые пары"));

        // Показываем только закрытые пары
        tradingPairsComponent.showOnlyClosedPairs();
    ...
    private void updateUI() {
        getUI().ifPresent(ui -> ui.access(() -> {
            try {
                ...
                tradingPairsComponent.updateClosedPairs();
            } catch (Exception e) {
