...
public class StablePairsScheduler {
    ...
    public CompletableFuture<Void> executeStablePairsSearchAsync() {
        ...
        try {
            ...
            // Итоговая статистика
            log.debug("🏁 Многопоточный автоматический поиск завершен:");
            log.debug("   📊 Успешных задач: {}/{}", successfulTasks.get(), totalTasks);
            log.debug("   ❌ Неудачных задач: {}", failedTasks.get());
            log.debug("   🔍 Всего проанализировано пар: {}", totalPairsAnalyzed.get());
            log.debug("   ✅ Всего найдено торгуемых пар: {}", totalPairsFound.get());
            log.debug("   ⚡ Использовано потоков: 5");
